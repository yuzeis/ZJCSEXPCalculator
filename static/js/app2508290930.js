(() => {
  ...
  const initExternalCountdown = async () => {
    ...
    const tasks = txt.split(/\r?\n/)
      ...
      .filter(Boolean)
      .map((line) => {
        const [title, ds] = line.split(',');
        if (!ds) return null;
        const [y, m, d, h, mi, s] = ds.trim().split('/').map(Number);
        const target = new Date(y, m - 1, d, h, mi, s);
        const div = document.createElement('div');
        div.className = CSS_CLASSES.COUNTDOWN_ITEM;
        dom.extCd.appendChild(div);
        return { el: div, title: String(title), target };
      })
      .filter(Boolean);
    ...
    const render = () => {
      ...
      for (let i = 0; i < tasks.length; i++) {
        ...
        if (diff > 0) allReached = false;
        t.el.textContent = diff <= 0 ? `${t.title} 已到达！` : `${t.title} 倒计时：${fmtDur(diff)}`;
      }
      if (allReached && runtime.extIntervalId) { clearInterval(runtime.extIntervalId); runtime.extIntervalId = 0; }
    };
